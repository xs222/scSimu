# From cscore package
#' CS-CORE with different mode of reweighting, for testing-only.
#'
#' @param X input count matrix data, columns are genes.
#' @param seq_depth input sequencing depth to be regressed out.
#' @params test mode of test, "Indep" or "General".
#' @params probs quantile of the dispersion parameter used to regularize the weight matrix
#' @params mode mode of reweighting schemes (1: negative binomial; 2: variance; 3: mean; 4: quadratic sequencing depth)
#' @params covar_weight if "regularized" use theta_median in the weight instad of theta

CscoreSimplifiedIRLS = function(X, seq_depth = NULL, test = "Indep", probs = 0.5, covar_weight = "regularized"){
   if (is.null(seq_depth)) {
    ...
    return(list(p_value = p_value, est = est, test_stat = test_stat))
}
