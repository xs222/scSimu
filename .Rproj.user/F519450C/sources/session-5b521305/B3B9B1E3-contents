...
# this is the corresponding ind simulation of /gpfs/gibbs/pi/zhao/xs282/validation/mean_cor/NB_simu_PNAS_and_ROSMAP_NC_Oli_11_13_2023.R
...
marginal_fit_fn <- paste0("marginal_fit/",dname,"_NC_Oli_marginal_fit.rds")
...
km_Ex5 <- readRDS(paste0('marginal_fit/',dname,'_NC_Oli_ks_fit_5.rds'))
...
simu_ROSMAP <- NB_copula(mu, gene_name, seq_depth, cell_name, alpha,
                         ind=T, seed=seed)
rm(gene_name)
if(!file.exists(paste0("real/oli/simu_IND/", dname,"/"))){
  dir.create(paste0("real/oli/simu_IND/", dname,"/"),recursive = T)
}
saveRDS(simu_ROSMAP, paste0("real/oli/simu_IND/", dname,"/simu_", prefix, ".rds"))
...
path2 <- paste0("real/oli/simu_IND/noise/",dname,"/",prefix,"/")
...
saveRDS(est_mat_ROSMAP, paste0("real/oli/simu_IND/", dname,"/est_cor_", prefix, ".rds"))
